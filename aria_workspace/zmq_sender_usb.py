# Aria SLAM ZMQ Sender - USB Version
#
# Connects to an Aria device via USB, captures stereo camera images, and streams raw (uncompressed) image data
...
#
# Configuration:
# - STREAMING_PROFILE: Streaming profile name for USB
...
CALIBRATION_SAVE_PATH = "/home/developer/aria_workspace/device_calibration_usb.json"  # Path to save calibration
...
RGB_UNDISTORT_FOCAL_LENGTH = 242    # Focal length for RGB undistorted output (None = use original from calibration)
...
class AriaUSBDeviceManager:
    ...
    def connect(self):
        ...
        # USB connection doesn't need IP configuration
    ...
    def stop_existing_sessions(self):
        ...
        streaming_state = self.streaming_manager.streaming_state
        recording_state = self.recording_manager.recording_state
        print(f"Current states - Streaming: {streaming_state}, Recording: {recording_state}")
    ...
    def start_streaming(self):
        ...
        # Configure for USB streaming
        streaming_config = aria.StreamingConfig()
        streaming_config.profile_name = self.streaming_profile
        streaming_config.streaming_interface = aria.StreamingInterface.Usb  # Key difference from WiFi version
...
# Reuse ZMQSender class from original zmq_sender.py - identical functionality
class ZMQSender:
...
def main():
    """Main execution function for Aria USB ZMQ sender."""
    ...
    try:
        ...
        if ENABLE_RGB_STREAM:
            stream_types += " + RGB"
        print(f"Subscribed to {stream_types} data stream via USB, starting ZMQ transmission ({image_type_str} images)...")
